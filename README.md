...
grep 命令用于查找文件中匹配指定字符串的行，并输出匹配的行。

grep 的语法格式：`grep [options] pattern [file]`，其中 options 可选，pattern 为要查找的字符串，file 为要查找的文件。

参数 file 为空或为 `-` 时表示从标准输入读取数据，此时 grep 命令通常与 [管道命令](#管道命令pipe) 一起使用，这也是本文将 grep 命令归类于“标准输入输出操作”下的原因。
